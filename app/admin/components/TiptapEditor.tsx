...
const Toolbar = ({ editor }: { editor: Editor | null }) => {
  ...
  return (
    <div className="flex flex-wrap gap-1 p-2 border-b bg-zinc-50 rounded-t-xl sticky top-0 z-10">
      ...
      <div className="w-px h-6 bg-zinc-300 mx-1 self-center" />
      <button type="button" onClick={addImage} className="p-2 text-zinc-600 hover:bg-zinc-200 rounded">
        <ImageIcon size={18} />
      </button>
...
export default function TiptapEditor({ content, onChange }: { content: string, onChange: (html: string) => void }) {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: 'prose prose-zinc focus:outline-none min-h-[400px] p-8 text-black max-w-none bg-white',
      },
    },
  });

  // Editor yüklenene kadar loading state göster
  if (!editor) {
    return <div className="min-h-[400px] border border-zinc-200 rounded-xl bg-zinc-50 animate-pulse" />;
  }

  return (
    <div className="border border-zinc-200 rounded-xl overflow-hidden bg-white shadow-sm transition-all">
